...
def run_vlasov2d():
    ...
    h = 0.5e-3
    geom = ibsimu.Geometry(ibsimu.MODE_2D, ibsimu.Int3D(241, 101, 1), ibsimu.Vec3D(0, 0, 0), h)
    ...
    pdb.trajectories_at_plane(tdata_exit, ibsimu.AXIS_X, 115e-3, [ibsimu.DIAG_Y, ibsimu.DIAG_YP])

    # 6. Plotting
    plt.figure(figsize=(12, 16))
    mesh_ext = [geom.origo(0)*1000, geom.max(0)*1000, geom.origo(1)*1000, geom.max(1)*1000]
    ...
    plt.subplot(4, 1, 3)
    # Draw electrodes
    plt.gca().add_patch(plt.Rectangle((0, 18), 20, 32, color='gray', alpha=0.3, label='Electrodes'))
